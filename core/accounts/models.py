...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    profileIFC = models.CharField(max_length=240, null=True, blank=True)
    base = models.CharField(max_length=5, null=True, blank=True, default='N/D')
...
class PirepsFlight (models.Model):
    ...
    event = models.CharField(choices=STATUS_EVENTOS, max_length=50)
    ...
    alternate_airport = models.CharField(max_length=50, blank=True, null=True)    
    route = models.TextField(blank=True, null=True)
    scores = models.FloatField(blank=True, default=0.5)
...
class Evento(models.Model):
    ...
    tipo = models.CharField(max_length=20, choices=TIPOS_DE_EVENTOS)
    aeroporto = models.CharField(max_length=200, null=True, blank=True)
    data = models.DateField(null=True, blank=True)
    hora = models.TimeField(null=True, blank=True)
    link = models.URLField(max_length=200, null=True, blank=True)    
    link_evento = models.URLField(max_length=200, null=True, blank=True)
...
class Agendamento(models.Model):
    STATUS_CHOICES = [
        ('Partida Verde', 'Partida Verde'),
        ('Pousou Verde', 'Pousou Verde'),
        ('Partida Amarelo', 'Partida Amarelo'),
        ('Pousou Amarelo', 'Pousou Amarelo'),
        ('Partida Vermelho', 'Partida Vermelho'),
        ('Pousou Vermelho', 'Pousou Vermelho'),
        ('Agendado Cinza', 'Agendado Cinza'),
        ('Estimado Verde', 'Estimado Verde'),
        ('Cancelado Vermelho', 'Cancelado Vermelho'),
    ]

    evento = models.ForeignKey(Evento, on_delete=models.SET_NULL, null=True, blank=True, verbose_name='Evento')
    origem = models.CharField(max_length=200, null=True, blank=True, verbose_name='Origem')
    ...
    numero_voo = models.CharField(max_length=200, null=True, blank=True, verbose_name='Número do Voo')
    hora_partida = models.CharField(max_length=200, null=True, blank=True, default="N/D", verbose_name='Hora de Partida')
    hora_chegada = models.CharField(max_length=200, null=True, blank=True, default="N/D", verbose_name='Hora de Chegada')
    aeronave = models.CharField(max_length=200, null=True, blank=True, verbose_name='Aeronave')
    gate = models.CharField(max_length=200, null=True, blank=True, default='N/D', verbose_name='Gate')
    status = models.CharField(max_length=200, blank=True, choices=STATUS_CHOICES, default='Agendado Cinza', verbose_name='Status')
    piloto = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name='Piloto')    
    data_criacao = models.DateTimeField(auto_now_add=True, verbose_name='Data de Criação')


    def __str__(self):
        return f"Agendamento: {self.numero_voo} - {self.origem} para {self.destino}"
